        ...
        for model, marker_fields in MARKER_MODEL_MAP.items():
            for marker_field in marker_fields:
                ...
                if values:
                    ...
                    warnings.warn(
                        f"No values found for marker '{marker_field}' (ID: {marker_obj.id})",
                        UserWarning,
                    )
...
def populate_mf_responses():
    Experience_Level_base_query = MFSchemeMasterInDetails.objects.filter(
        status="Active"
    )
    ...
    if "color" in Experience_Level:
        Experience_Level_base_query = Experience_Level_base_query.annotate(
            color=Subquery(
                MFSchemeMaster.objects.filter(schemecode=OuterRef("schemecode")).values(
                    "color"
                )[:1]
            )
        )
    ...
    if "setup_date" in Experience_Level:
        Experience_Level_base_query = Experience_Level_base_query.annotate(
            setup_date=Subquery(
                MFAMCMaster.objects.filter(amc_code=OuterRef("amc_code")).values(
                    "setup_date"
                )[:1]
            )
        )
    ...
    if "type" in Experience_Level:
        Experience_Level_base_query = Experience_Level_base_query.annotate(
            type=Subquery(
                MFTypeMaster.objects.filter(type_code=OuterRef("type_code")).values(
                    "type"
                )[:1]
            )
        )

    result_query1 = Experience_Level_base_query.values(
        "schemecode", "color", "setup_date", "type", "Incept_date"
    )
    ...
    if "sharpe_y" in Risk_Tolerance:
        Risk_Tolerance_base_query = Risk_Tolerance_base_query.annotate(
            sharpe_y=Subquery(
                MFRatios1Year.objects.filter(schemecode=OuterRef("schemecode")).values(
                    "sharpe_y"
                )[:1]
            )
        )
    ...
    Investment_Duration_base_query = MFSchemeMasterInDetails.objects.filter(
        status="Active"
    )
    ...
    Decision_Making_Style_base_query = MFSchemeMasterInDetails.objects.filter(
        status="Active"
    )
    ...
    Emotional_Reactions_to_Market_Changes_base_query = (
        MFSchemeMasterInDetails.objects.filter(status="Active")
    )

    Emotional_Reactions_to_Market_Changes = [
        "_52weekhhigh",
        "gross",
        "ytdret",
        "adjustednav_c",
    ...
    if "_52weekhhigh" in Emotional_Reactions_to_Market_Changes:
        Emotional_Reactions_to_Market_Changes_base_query = (
            Emotional_Reactions_to_Market_Changes_base_query.annotate(
    ...
    Research_and_Professional_Advice_base_query = (
        MFSchemeMasterInDetails.objects.filter(status="Active")
    )

    Research_and_Professional_Advice = [
        "sebi_reg_no",
        "trustee_name",
        "cust_name",
        "IsPurchaseAvailable",
    ...
    if "sebi_reg_no" in Research_and_Professional_Advice:
        Research_and_Professional_Advice_base_query = (
            Research_and_Professional_Advice_base_query.annotate(
    ...
    Sectoral_and_Investment_Preferences_base_query = (
        MFSchemeMasterInDetails.objects.filter(status="Active")
    )
    ...
    if "asset_type" in Sectoral_and_Investment_Preferences:
        Sectoral_and_Investment_Preferences_base_query = (
            Sectoral_and_Investment_Preferences_base_query.annotate(
                asect_type=Subquery(
            ...
            for key, value in item.items():
                if key == "schemecode":
                    ...
                    if isinstance(value, (datetime, date)):
                        ...
                        warnings.warn(
                            f"Unexpected type {type(value)} for marker '{key}'"
                        )
                        continue
                    ...
                    for option in option_obj:
                        if option.option.startswith("[") and option.option.endswith(
                            ...
                            if len(option_ranges) == 2:
                                if (
                                    float(option_ranges[0])
                                    <= final_value
                                    <= float(option_ranges[1])
                                ):

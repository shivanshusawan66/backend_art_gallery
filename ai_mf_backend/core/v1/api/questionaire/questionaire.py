...
def get_section_wise_questions(section_request: SectionRequest):
    try:
        ...
        if not current_section:
            return JSONResponse(
                status_code=404,
                content={"status_code": 404, "detail": "Section not found."},
            )
        ...
        return JSONResponse(
            status_code=500, content={"status_code": 500, "detail": "Database error"}
        )
    except Exception as e:
        # Log the error for debugging
        print(f"Unexpected error: {str(e)}")  # Consider using logging instead
        return JSONResponse(
            status_code=500, content={"status_code": 500, "detail": str(e)}
        )

...
from ai_mf_backend.models.v1.api.blog_comment_report import (
    ReportCreateRequest,
    ReportResponse,
)
...
async def report_comment_or_reply(
    ...
    try:
        ...
        user = await sync_to_async(
            lambda: UserContactInfo.objects.filter(email=email).first()
        )()
        if not user:
        ...
        valid_ids = await sync_to_async(
            lambda: list(BlogCommentReportType.objects.values_list("id", flat=True))
        )()
        ...
        try:
            report_type = await sync_to_async(
                lambda: BlogCommentReportType.objects.get(id=request.report_type_id)
            )()
        ...
        if request.comment_id:
            comment = await sync_to_async(
                lambda: BlogComment.objects.filter(id=request.comment_id).first()
            )()
            if not comment:
        ...
        if request.reply_id:
            reply = await sync_to_async(
                lambda: BlogCommentReply.objects.filter(id=request.reply_id).first()
            )()
            if not reply:
                response.status_code = 404
                return ReportResponse(
        ...
        report = await sync_to_async(
            lambda: BlogCommentReport.objects.create(
                user=user, comment=comment, reply=reply, report_type=report_type
            )
        )()
